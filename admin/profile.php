...
if(isset($_POST['update'])){
    ...
    $stmtCheck->execute([$email, $admin_id]);
    if($stmtCheck->rowCount() > 0){
        ...
        if(!empty($new_pass)){
            ...
            $updated = $stmtUpdate->execute([$name, $email, $passwordHash, $admin_id]);
        } else {
            $stmtUpdate = $conn->prepare("UPDATE users SET name = ?, email = ? WHERE id = ?");
            $updated = $stmtUpdate->execute([$name, $email, $admin_id]);
        }
    ...
    <div class="card">
        <form method="POST">
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control" readonly value="<?= ucfirst($admin['role']); ?>">
            </div>
            <div class="mb-3">
                <label class="form-label">Status</label>
                <input type="text" class="form-control" readonly value="<?= ucfirst($admin['status']); ?>">
            </div>
            <div class="mb-3">
